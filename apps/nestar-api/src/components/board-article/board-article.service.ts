...
export class BoardArticleService {
	...
	public async getBoardArticles(memberId: ObjectId, input: BoardArticlesInquiry): Promise<BoardArticles> {
		...
		if (input.search?.memberId) {
			match.memberId = shapeIntoMongooseObjectId(input.search.memberId); //aynan bir memberning articlelarini kurmoqchi bulsak, memberId dan ham kurishimiz mumkin.
		}
	...
	public async removeBoardArticleByAdmin(articleId: ObjectId): Promise<BoardArticle> {
		...
		const result = await this.boardArticleModel.findOneAndDelete(search).exec(); //DB dan butunlay uchirish
